...
export const getCourseToCertificate = async (req, res) => {
  try {
    ...
    const lessons = await prisma.lessons.findMany({
      where: { course_id: { in: courseIds } },
      select: { lesson_id: true, course_id: true },
    });

    const progress = await prisma.userProgress.findMany({
      where: { user_id: userId },
      select: { course_id: true, lesson_id: true, is_completed: true },
    });
    ...
    const mergedData = userCourses.map((enrollment) => {
      const totalLessons = lessonsByCourse[enrollment.course_id]?.size || 0;
      const completedLessons =
        completedByCourse[enrollment.course_id]?.size || 0;
      ...
      const certificate = certificates.find(
        (cert) => cert.course_id === enrollment.course_id
      );

      return {
        course_id: enrollment.course_id,
        title: course?.title || null,
        thumbnail: course?.course_img || null,
        ...
        certificateRequested: Boolean(certificate),
        certificateStatus: certificate?.status || "not_requested",
    ...
    console.error("Error in getCourseToCertificate:", error);
