...
const findAndAssignSupportTicket = async (ticket_id) => {
  ...
  const sortedSupportUsers = supportUsersWorkLoad.sort((a, b) => a.workload - b.workload);
...
export const createSupportTicket = async (req, res) => {
  try {
    ...
    const chatLink = `${process.env.BASE_URL}/support/users/chat?access_token=` + token;

    process.nextTick(async () => {
      try {
        const assignedSupportTicket = await findAndAssignSupportTicket(supportTicket.ticket_id);
        ...
        console.error(`Error assigning ticket #${supportTicket.ticket_id}:`, error);
      }
    });
