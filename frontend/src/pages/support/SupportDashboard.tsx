  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex flex-1 h-[calc(100vh-4rem)]">
        {showSidebar && (
          <SupportDashboardSidebar tickets={tickets} />
        )}

        <div className="flex-1 flex overflow-hidden">
          <div className={`${selectedTicket ? 'hidden md:block' : 'block'} w-full md:w-1/3 border-r border-gray-200 bg-white overflow-y-auto`}>
            <TicketList tickets={tickets} selectedTicket={selectedTicket} setSelectedTicket={setSelectedTicket} />
          </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="mt-1 flex items-center text-sm text-gray-500">
                      ...
                      <span className="mx-1">·</span>
                      <span>Last updated {formatTime(selectedTicket.updated_at)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    <div className="h-10 w-10 rounded-full bg-gray-200 flex-shrink-0 overflow-hidden">
                      {selectedTicket.user.avatar ? (
                        <img src={selectedTicket.user.avatar} alt={selectedTicket.user.full_name} className="h-full w-full object-cover" />
                      ) : (
                        <User className="h-5 w-5 text-gray-500 m-auto" />
                      )}
                    </div>
                    <div className="ml-3">
                      <p className="text-sm font-medium text-gray-900">{selectedTicket.user.full_name}</p>
              ...
              <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
                <div className="space-y-4">
                  {selectedTicket.messages.map((message, index) => (
                    ...
                    >
                      <div
                        className={`max-w-3/4 rounded-lg px-4 py-2 ${message.sender === 'user' ? 'bg-white border border-gray-200' : 'bg-blue-100 border border-blue-200'}`}
                      >
                        <div className="text-sm text-gray-800">{message.message}</div>
                        <div className={`mt-1 text-xs ${message.sender === 'user' ? 'text-gray-500' : 'text-blue-600'}`}>
                    ...
                    <div className="border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-500 shadow-sm ">
                      ...
                      <div className="flex justify-between items-center bg-gray-100 px-4 py-2">
                        ...
                        <button
                          ...
                          className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white ${replyMessage.trim() && !isSending ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-300 cursor-not-allowed'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500`}
                        >
                          {isSending ? 'Sending...' : (
                            <>
