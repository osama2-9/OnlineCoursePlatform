...
const UsersChatPage = () => {
  ...
  const verifyAccessToken = async () => {
    try {
      ...
      toast.error(error.response?.data?.error || "Failed to verify access");
      setIsAuthenticated(false);
    } finally {
      setIsLoading(false);
    }
  ...
  const loadTicketData = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        if (res.data.SupportTicketMessage && Array.isArray(res.data.SupportTicketMessage)) {
          }
      }
    } catch (error: any) {
      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            Invalid or expired access token. Please use the link provided in your email to access this support chat.
          </p>
          <a
            href="/"
          ...
          <div className="border-b p-4 md:p-6">
            <div className="flex justify-between items-start">
              ...
              <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
                ...
                {supportTicket.status === 'closed' && 'Closed'}
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="flex items-center text-gray-600">
                ...
                <span>Created: {new Date(supportTicket.created_at).toLocaleDateString()}</span>
              </div>
              
              <div className="flex items-center text-gray-600">
                <Headphones size={16} className="mr-2" />
                <span>Agent: {supportTicket.supportUser.fullname || 'Unassigned'}</span>
              </div>
            </div>

            <div className="mt-4 p-3 bg-gray-50 rounded-md">
              <div className="text-sm text-gray-500 mb-1">Original description:</div>
          ...
          <div className="flex flex-col h-[500px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {supportTicket?.messages?.map((msg) => (
                <div
                  key={msg.message_id}
                  className={`flex ${msg.user_id == user?.userId ? 'justify-end' : 'justify-start'}`}
                >
                  <div
                  ...
                  >
                    <div className="text-sm mb-1 flex items-center">
                      {msg.user_id == user?.userId ? 'You' : supportTicket.supportUser.fullname}
                      <span className="text-xs mx-2 opacity-75">•</span>
                      <span className="text-xs opacity-75">{new Date(msg.sent_at).toLocaleTimeString()}</span>
                    </div>
                    <p>{msg.message}</p>
                  </div>
                </div>
            ...
            <div className="border-t p-4">
              <form onSubmit={sendMessage} className="flex items-center space-x-2">
                ...
                <button
                  type="submit"
                  disabled={supportTicket.status === 'closed' || !message.trim()}
                  className={`${
                    supportTicket.status === 'closed' || !message.trim() 
                    ? 'bg-gray-400' 
                    : 'bg-blue-600 hover:bg-blue-700'
                  } text-white rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2`}
                >
                  <Send size={20} />
                </button>
              </form>
